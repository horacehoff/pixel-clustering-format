...
pub fn compare(args:Vec<String>) {
    ...
    fn test_entry(entry: DirEntry,name:&mut Vec<String>, original: &mut Vec<usize>, compressed: &mut Vec<usize>, compressed_lossy: &mut Vec<usize>) {
        if entry.file_name().to_str().unwrap().to_string().contains("credits") {
            return;
        }
    ...
    for entry in entries {
        ...
        if fuck.file_name().to_str().unwrap().to_string().starts_with(args.get(1).unwrap()) {
            test_entry(fuck, &mut name, &mut original, &mut compressed, &mut compressed_lossy);
        }
    }
    println!("{name:?}");
    println!("{original:?}");
